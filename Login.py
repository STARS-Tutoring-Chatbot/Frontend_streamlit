...
#used to hide the sidebar in streamlit
hide_stSidebar = """        
    <style>
    ...
    if selection == 'Login':
        ...
        if email != '' and password != '':
            if is_valid_email(email) != True:
                st.write('Please enter a valid FIU email address')
            if len(password) < 7:
        ...
        if email != '' and password != '':
            ...
            if is_valid_email(email) and len(password) > 7:
                if collection.find_one({'email': email}) is None:
                    if submitted == True:
                        collection.insert_one({'email': email, 'password': password, 'username': username, 'uploaded_image': ''})
                        credentials.email = email
                        credentials.password = password
                        credentials.username = username
                        st.switch_page("pages/streamlit_app2.py")
                else:
                    st.write('Email is already Registered')
        else:
            if submitted == True:
                st.write('Email and password are required.')
